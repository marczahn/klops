...
// import { GameHandle } from '../models/interfaces';
...
//
// 	return {
// 		getState: (): IGame => toExternalGameState(state),
// 		rotate: () => {
...
// 			pause()
// 		},
// 		addListener: (listener: (state: IGame, action: string) => void) => {
...
//
// const toExternalGameState = (state: GameState): IGame => ({
// 	id: state.id,
...
// }
//
// const addListener = (state: GameState, listener: (state: IGame, action: string) => void): GameState => {
...
// }
//
export const createMatrix = (cols: number, rows: number): Matrix => {
	const out: number[][] = []
